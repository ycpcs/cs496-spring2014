...
In this lab we will create two different tab views for the NATO countries (stored as a *string-array* in *strings.xml*).

**Set up tab view**

First set up the tabs

> -   In *main.xml* set up a vertical *LinearLayout* for the tabs using a *TabHost*, *TabWidget*, and *FrameLayout* tags. NOTE: Be sure to give your *TabHost* an *id* of <*@android>:id/tabhost\*, the *TabWidget* an *id* of <*@android>:id/tabs\*, and the *FrameLayout* an *id* of <*@android>:id/tabcontent\*.
> -   In the *MainTabView* activity class, change the base class to *TabActivity*. Then add code to *onCreate()* to set up two tabs: one for the *ListTabActivity* and one for the *GridTabActivity*. Set the initial tab to the *ListTabActivity* tab. Simply use the *ic\_tab1* icons for both tabs (later you may wish to experiment with changing the icons for each tab).

At this point you should have a basic tabbed view (with a simple TextView placeholder in each tab) which looks similar to

> ![image](images/lab15/tabview.png)
...
> -   In the *ListTabActivity* activity class, change the base class to *ListActivity* and remove the *setContentView()* call. Then add code to *onCreate()* to build a *ListAdapter* from the *string-array* named *countries* in the *strings.xml* resource file, set the list adapter to this one, and then register an item click callback for the list view that displays a simple *Toast* message for the item selected.

Now the list tab should appear as

> ![image](images/lab15/listview.png)

**Set up grid tab**

To set up the grid tab

> -   In *grid\_layout.xml* replace the *TextView* with a *GridView* (use the attributes from the example in [lecture15](../lecture/lecture15.html)).
...
> ![image](images/lab15/gridview.png)

Advanced Views
==============

We can enhance the appearance of our list/grid views by incorporating images into the items within the views. However to do this required creating our own adapter subclasses from the [BaseAdapter](http://developer.android.com/reference/android/widget/BaseAdapter.html) base class. We must then override the *getView()* method which returns the view for each item to customize the returned views with whatever content we wish.
...
Create a new class named *ImageAdapter* which extends the *BaseAdapter* class. Add the following private fields to the class (which stores references to the images).
...
The important method is the *getView()* which takes as a parameter the *position* of the particular cell which we use to access the corresponding image from the resources. We then use the image to create an *ImageView* which is displayed in the grid cell.
...
> ![image](images/lab15/imagegrid.png)
...
Create a file named *list.xml* in the *res/layout* directory that contains the layout for the rows. Since we want to have an image and country name text field, our layout might be:
...
which create a local *LayoutInflater* object which will parse out the XML layout file, a *RowData* object (to be defined later) which will create each individual row, a *Vector* of *RowData* objects for the entire list which will be used to initialize the custom adapter, and two local arrays containing the image references with corresponding text strings.
...
Now add a local private class for the custom adapter named *Custom Adapter* that extends *ArrayAdapter\<RowData\>* as
...
where the *getView()* method parses up the row XML layout using the *LayoutInflater* and assigns the appropriate data to the different tags in the view using a *ViewHolder* object defined below.
...
*\* Instantiate adapter for the list*\*
...
> ![image](images/lab15/imagelist.png)
